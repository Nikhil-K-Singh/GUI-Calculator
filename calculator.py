...
sum1 = Calc()   #create an object of Calc class
root = Tk()     #create a window 
calc = Frame(root)
calc.grid()     #display a frame
...
#we have initialised the output display
...
#defining the buttons now for input interface
...
mult["command"] = lambda: sum1.operation("multiply")
mult.grid(row = 2, column = 3,  padx=1, pady = 1)

subtract = Button(calc,height =2,width=4,padx=10, pady = 10, text = "-",bg="steel blue")
...
power["command"] = lambda: sum1.operation("power")
power.grid(row=2,column = 4,padx=1,pady=1)

rooting = Button(calc, height=2, width=4, padx=10, pady=10, text="y-\/x", bg = "green")
rooting["command"] = lambda: sum1.operation("rooting")
rooting.grid(row=2, column=5, padx=1, pady=1)
...
point = Button(calc,height =2,width=4,padx=10, pady = 10, text = ".",bg="gray")
point["command"] = lambda: sum1.num_press(".")
point.grid(row = 4, column = 1, padx=1, pady = 1)

neg= Button(calc,height =2,width=4,padx=10, pady = 10, text = "+/-",bg="gray")
neg["command"] = sum1.sign
neg.grid(row = 4, column = 2,  padx=1, pady = 1)


clear = Button(calc,height =2,width=4,padx=10, pady = 10, text = "C",bg="gray")
clear["command"] = sum1.clear
clear.grid(row = 1, column = 4,  padx=1, pady = 1)

all_clear = Button(calc,height =2,width=4,padx=10, pady = 10, text = "AC",bg="gray")
all_clear["command"] = sum1.all_clear
all_clear.grid(row = 1, column = 5, padx=1, pady = 1)

equals = Button(calc,height =6,width=4,padx=10, pady = 10, text = "=",bg="gray")
